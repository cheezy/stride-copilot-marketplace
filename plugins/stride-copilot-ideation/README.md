...
This plugin is a faithful port of [`cheezy/stride-ideation`](https://github.com/cheezy/stride-ideation) to GitHub Copilot CLI — v0.1.0 ported upstream v0.7.0, **v0.2.0 ports the upstream v0.8.0 feature set** (per-round recap, "I'm not sure" uncertainty path, profile recommendation, `--input` seed, draft autosave/resume, reviewer-findings decision, and the `stridify` preview-and-approval gate with `--yes`), **v0.3.0 ports the upstream challenge gate** (assumption-confidence audit, blind-spot scan, two alternatives, cost/risk/complexity/timeline trade-off — folded into the optional **Design challenge** section; see "Session experience" above), and **v0.4.0 hardens the decomposition-and-ship pipeline around the Stride review queue's five per-task scored fields** (decomposer contract names and demonstrates `acceptance_criteria` / `testing_strategy` / `security_considerations` / `pitfalls` / `patterns_to_follow`; the stridify pipeline stamps `created_by_agent` on each shipped goal; `validate_batch.py` gains a `varchar(255)` length check plus advisory scored-field warnings; every calibration fixture task carries all five scored fields) and completes the port of the core round-loop protocol to the Copilot selection primitive. The protocol — round-based question batching, hard-gated sections, advisory reviewer pass, decomposer dispatch with bounded retry, retry-exhaustion fallback, source_spec stamping, validator-before-commit, never-retry-POST — is preserved verbatim. The differences are mechanical adaptations:
...
The reviewer agent rubric and the lib/ helpers' Python scripts track upstream closely. As of **v0.4.0** the decomposer agent prompt, the calibration fixtures, and `lib/validate_batch.py` carry the port's own review-queue hardening — the decomposer contract names the five scored fields and adds `security_considerations` to the skeleton and every worked example, every fixture task carries all five scored fields, and the validator adds a `varchar(255)` length check plus advisory scored-field warnings — the same guardrails upstream carries, with all command/skill-name references adapted to this Copilot port (which is why these files are no longer byte-identical to upstream).
