...
8. **Return Structured Review**:
   ...
   - The JSON object has these top-level fields (all required unless explicitly marked OPTIONAL, snake_case throughout):
     - `schema_version`: string. Always `"1.7"` for this prompt version. *(Bumped from `"1.6"` by W2156, which added the `cosmetic` key to `issues[]` entries. Nothing gates on the value: the server validates it as a semver-shaped string only.)*
     ...
     - **The `cosmetic` finding class — what it is.** Set `cosmetic: true` only when **all three** gates hold: (1) the *finding's* claim is correct, (2) the *artifact* it points at asserts nothing that is itself false, and (3) **with those two satisfied, the subject must then be purely presentational.** The second gate is the one that is easy to miss, so here are the misses: a doc saying "prints six values" where seven print; a comment placing an assert below a write that sits above it; a heading reading "Two limits" above three. Your finding is right in every one of those — what makes each **substantive** is that the artifact states something false. **A false statement of fact is never cosmetic, however small.** **Gate three is the one that does the work, and it is a requirement, not a description.** Purely presentational subjects are line wrapping, column width, word and line counts, phrasing preference, and the ordering of otherwise-equivalent prose. **Apply the test, not the examples** — that list illustrates gate three, it does not define it, and clearing gates one and two does not clear gate three. **Default deny: set `cosmetic` `false`, or omit it, on everything else.** A finding whose subject is a data structure, a configuration entry, a trigger condition, a test primitive or an assertion is not presentational, so gate three refuses it even when the first two gates pass — which they often will, because such an artifact states no proposition for gate two to bite on.
     - **Gate three is not present-tense.** "Nothing behaves differently *today*" does not satisfy it. A skip-list entry that currently matches nothing, a trigger stated more broadly than its example, two counting primitives that agree on today's content — each is a latent defect whose subject is machinery, not appearance, and each is **substantive**.
     - **Location, keyed on the property rather than on an operation.** The question is never which edit you made; it is **whether anything reads the thing you changed** — its bytes, its line positions, its ordering, its structure. Re-wrapping a paragraph nothing parses is cosmetic. A re-wrap inside executable content — a jq expression, a shell heredoc, a code fence something greps or extracts — changes what runs. So does **any** whitespace or structural edit at a position something depends on, including inserting or removing a blank line, changing a heading level, or moving a line between paragraphs, in a file whose layout is read by line arithmetic. Naming only re-wraps and re-orderings would let a blank line through: this port's own suite does line arithmetic over markdown prose, so a blank line in prose can break an assertion.
     - **What `cosmetic` does NOT do.** It does not change `severity`. It does not change `category`. It does not change `status` — so the `status` rule above needs no exclusion for it. It does not remove the finding from `issues[]`, from your report, or from anything downstream. **The single thing it changes is the orchestrator's re-review disposition.** It is therefore not a carve-out, an exclusion, or a suppression, and it amends none of this file's exhaustive-carve-out wording.
     - **Never a downgrade.** `cosmetic: true` on a substantive finding — anything where code or a contract would behave differently, or a reader would be misled — is a **reviewer defect**, not a judgement call. The test is not "is this small" but "is the only thing wrong here how it looks". A one-character typo that inverts a meaning is substantive; a line four columns too wide is cosmetic. **`minor` and `cosmetic` are orthogonal, not synonyms:** `cosmetic` only ever sits on a `minor`, and a `minor` can be substantive and still buy a review round.
     - **Three conditions make `cosmetic: true` invalid output.** The severity is anything other than `minor` — which covers **`critical` and `important` alike** — or `category` is `"security"`, or `cosmetic` is not a real boolean (no coercion: `1`, `"yes"` and `"true"` are all refused). **This prohibition is stated, not mechanically checked.** The reference enforces it with a `cosmetic_shape_ok` pin on its two extraction paths; this port has no extraction pin to attach one to, and no hook here can observe a reviewer's block, so **nothing refuses the submission**. The three conditions bind because you follow them, and because the completion self-check in `stride-completing-tasks` re-reads them before submission — prose and a self-report, never a pin. Read that as the enforcement class it is: violating one costs a caught defect at the gate, not a rejected payload.
     - **Stated residual — the category-keyed edge, and why this port is not where the reference is.** The refusal above keys on the literal `category: "security"` string, so a security-relevant finding filed at `minor` under `code_quality` or `pitfall` can still carry `cosmetic: true`; no signal in the block distinguishes it. Be exact about the reference here rather than borrowing its authority: what the reference discloses is that its `cosmetic_shape_ok` pin reaches a flag's **type and co-ordinates** — severity and category — and never a finding's subject matter, and the cheapest abuse it names is relabelling an ordinary substantive `minor`. It does **not** record this mis-filed-category edge as accepted, so do not read that into it. What is verifiable is narrower and still the point: the reference's own recording prohibition keys on the literal `category: "security"` string. **This port's does not.** The security rule in `stride-workflow` Step 5 keys on the finding's **subject matter, however the reviewer categorized it** — `code_quality` and `pitfall` included — and the completion hard gate re-applies that test to every finding before submission, independent of any `cosmetic` flag. A mis-filed security finding marked cosmetic is therefore still caught before submission: it is fixed or escalated at the gate — **on the same prose-and-self-report terms as the cosmetic and review-round bullets there**, since nothing in this port reads a reviewer's block. What `cosmetic` can still spare is the re-review *dispatch* that might have surfaced it a second time — the flag changes disposition only, so the finding stays in `issues[]` and stays in front of that gate. **That is a narrower residual than the reference's**, and it is stated rather than left to inference.
     - **Canon-governed — entry `cosmetic-finding-class` in `stride/docs/port-canon.md`.** That entry registers the paragraphs above as one rule every port must carry. A change to their substance owes a version bump in **two** places before the next release: that entry in the canon, and this file's own `<!-- canon:cosmetic-finding-class ... -->` anchor above.
     ...
     - **Canon-governed — entry `verdict-note` in `stride/docs/port-canon.md`.** Edit what the two rules above state and two version numbers move with them, both before release: the canon entry's, and the one on the anchor immediately above.
