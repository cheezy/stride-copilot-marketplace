...
|--------|---------|-------------|
| [`stride-copilot`](plugins/stride-copilot) | 2.36.0 | Task lifecycle skills and custom agents for Stride kanban: claiming, completing, and creating tasks and goals for AI agents in GitHub Copilot CLI. |
...
| [`stride-copilot-lite`](plugins/stride-copilot-lite) | 0.6.0 | Produce Stride-shaped goal and task markdown documents on disk from a free-text prompt — no API calls, no kanban auth. **(v0.4.0)** the hook layer now fires on Copilot CLI; skill directories renamed (breaking). |
